...
// ── Timing view — 3-column layout ─────────────────────────────────────────────
function TimingView() {
  return (
    <div className="grid gap-3 h-full" style={{ gridTemplateColumns:'300px 1fr 280px', minHeight:'580px' }}>
      {/* LEFT — Timing Tower (tall) */}
      ...
      {/* RIGHT — Race Control Feed (tall) */}
      <div className="row-span-2" style={{ minHeight:'580px' }}>
        <RaceControlFeed />
      </div>
...
// ── Telemetry view — 2-column layout ──────────────────────────────────────────
function TelemetryView() {
  return (
    <div className="grid gap-3" style={{ gridTemplateColumns:'280px 1fr', minHeight:'620px' }}>
      <Leaderboard />
      <TelemetryWorkspace />
    </div>
  );
...
function StrategyView() {
  return (
    <div className="grid gap-3" style={{ gridTemplateColumns:'280px 1fr 280px', minHeight:'580px' }}>
      <Leaderboard />
      <TireStrategy />
      <RaceControlFeed />
    </div>
...
export default function App() {
  const { activeView, setActiveView, currentLap, totalLaps, sessionName, isLive } = useRace();
